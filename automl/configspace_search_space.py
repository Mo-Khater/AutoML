...
def get_classification_configspace(
    ...
    return _build_configspace(get_classification_components(allowed_models=allowed_models))


def get_regression_configspace(
    allowed_models: list[str] | None = None,
) -> ConfigurationSpace:
    return _build_configspace(get_regression_components(allowed_models=allowed_models))
...
def sample_regression_configuration(
    configspace: ConfigurationSpace,
) -> Configuration:
    return configspace.sample_configuration()
...
def _build_configspace(components: dict[str, object]) -> ConfigurationSpace:
    ...
    for component_name in selected_models:
        ...
        conditions = component.build_conditions({"model_name": model_name, "params": params})
        if conditions:
            cs.add(conditions)
